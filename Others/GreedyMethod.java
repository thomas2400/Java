...
public class GreedyMethod {
  ...
  public static void main(String[] args) {
    ...
    int n = itemlist.size();
    double AR = greedyMethod(itemlist, bagsize, n);
   ...
   */
  public static double greedyMethod(ArrayList<Item> items, int bagsize, int n)
    throws IllegalArgumentException {
    int iw, iv;
    double fraction, totalVal = 0d;
    if (items.isEmpty()) throw new IllegalArgumentException();
    items.sort(
      new Comparator<Item>() { // rearrange the list of items user keyed in according to its cost in descending order
        public int compare(Item i1, Item i2) {
          return Double
            .valueOf(i2.getCost())
            .compareTo(Double.valueOf(i1.getCost()));
        }
      }
    );
    ...
    while (iterator.hasNext()) {
      ...
      if (bagsize - iw >= 0) { // if the space available is not 0 , then add in the items that is fit with the space available currently
        ...
        fraction = ((double) bagsize / (double) iw);
        totalVal += (iv * fraction);
        bagsize = (int) (bagsize - (iv * fraction));
        break;
      }
    }
    return totalVal;
