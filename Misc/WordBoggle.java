...
public class WordBoggle {
  ...
  public static List<String> boggleBoard(char[][] board, String[] words) {
    ...
    for (int i = 0; i < board.length; i++) for (
      int j = 0;
      j < board[i].length;
      j++
  ...
  public static void explore(
    ...
    for (Integer[] neighbor : neighbors) explore(
      neighbor[0],
      neighbor[1],
      board,
      trieNode,
  ...
  public static List<Integer[]> getNeighbors(int i, int j, char[][] board) {
    ...
    if (i > 0 && j < board[0].length - 1) neighbors.add(
      new Integer[] { i - 1, j + 1 }
    );

    if (i < board.length - 1 && j < board[0].length - 1) neighbors.add(
      new Integer[] { i + 1, j + 1 }
    );

    if (i < board.length - 1 && j > 0) neighbors.add(
      new Integer[] { i + 1, j - 1 }
    );
