...
public class RangeInSortedArray {
  ...
  public static void alteredBinSearch(
    ...
    if (nums[mid] > key) alteredBinSearch(
      nums,
      key,
      left,
      mid - 1,
      range,
      goLeft
    ); else if (nums[mid] < key) alteredBinSearch(
      nums,
      key,
      mid + 1,
      right,
      ...
      if (goLeft) {
        if (mid == 0 || nums[mid - 1] != key) range[0] =
          mid; else alteredBinSearch(nums, key, left, mid - 1, range, goLeft);
      } else {
        if (mid == nums.length - 1 || nums[mid + 1] != key) range[1] =
          mid; else alteredBinSearch(nums, key, mid + 1, right, range, goLeft);
      }
    }
  }
  ...
  public static void alteredBinSearchIter(
    ...
    while (left <= right) {
      int mid = (left + right) / 2;
      if (nums[mid] > key) right = mid - 1; else if (nums[mid] < key) left =
        mid + 1; else {
        if (goLeft) {
  ...
  public static int getLessThan(int[] nums, int key, int left, int right) {
    ...
    while (left <= right) {
      int mid = (left + right) / 2;
      if (nums[mid] > key) right = mid - 1; else if (nums[mid] <= key) {
